...
class OrdLogEntry(OrdLogNamedTuple):
    ...
    def print_header(cls):
        print("action, exchange_ts, price, amount, exchange_id")
    def print(self):
        if is_available(self.actions_mask,OrdLogEntry.ActionFlag.ADD):
            print("{},{},{},{}".format("place_order", self.exchange_timestamp, self.order_price, self.amount, self.exchange_order_id))
        if is_available(self.actions_mask,OrdLogEntry.ActionFlag.CANCELED):
            print("{},{},{},{}".format("cancel_order", self.exchange_timestamp, self.order_price, self.amount, self.exchange_order_id))
...
class QshFileHeader:
    ...
    streams_count = None
    def __repr__(self):
        return "qsh(ver={}, app={}, created_at={}, streams_count={}, comment={})".format(self.version,self.application,self.created_at,self.streams_count,self.comment)
...
class QshFile:
    ...
    def read(self, length):
        ...
        if result == six.b('') or result=='' or len(result)<length:
            raise EOFError
    ...
    def read_ord_log_data(self):
        ...
        if availability_mask!=0:
            ...
            return True, ord_log_entry, aux_info_entry, self.external_quotes, deal_entry
        else:
...
if __name__=="__main__":
    import sys
    if len(sys.argv)<1:
        print("Usage: python3 qsh_read.py FILE.ordlog.qsh")
    with open(sys.argv[1]) as f:
        print("#",f.header)
        stream_type, instrument_code = f.read_stream_header()
        print("#", "qsh_stream(stream_type={}, instrument_code={})".format(stream_type, instrument_code))
        if stream_type==StreamType.ORD_LOG:
            OrdLogEntry.print_header()
            while not f.is_eof():
                ts, stream = f.read_frame_header()
                print("#", "qsh_frame(ts={},stream={})".format(ts,stream))
                while not f.is_eof():
                    more, ord_log_entry, aux_info_entry, external_quotes, deal_entry = f.read_ord_log_data()                
                    print("more=",more)
                    if not more:
                        break
                    ord_log_entry.print()